...
OUTPUT_PATH = TOPIC_DIR / "Proposal - Pleasant.docx"
...
def add_paragraph(document: Document, text: str, bold_prefix: str | None = None):
    paragraph = document.add_paragraph()
    if bold_prefix:
        label_run = paragraph.add_run(bold_prefix)
        label_run.bold = True
...
def main() -> None:
    ...
    title = (
        "Design and Implementation of a Maternal Health Decision Support System for "
        "Primary Health Centre Staff to Flag High-Risk Pregnant Women Using Simple "
        "Clinic Records in Ugbomoro"
    )

    intro = document.add_paragraph()
    intro.alignment = WD_ALIGN_PARAGRAPH.CENTER
    intro_run = intro.add_run("PROJECT PROPOSAL DRAFT")
    intro_run.bold = True
    intro_run.font.name = "Times New Roman"
    intro_run.font.size = Pt(14)

    add_heading(document, "Student Details / Title", level=1)
    details = [
        ("Name: ", "Pleasant"),
    ...
    for label, value in details:
        add_paragraph(document, value, bold_prefix=label)
    ...
    add_heading(document, "Background Study", level=1)
    for paragraph in [
    ...
    add_heading(document, "Aim/Objectives", level=1)
    add_heading(document, "Aim", level=2)
    add_paragraph(
        document,
    ...
    add_heading(document, "Objectives", level=2)
    add_numbered_items(
        document,
        [
            "Build a desktop-first web dashboard for PHC staff.",
            "Support the registration of pregnant women and the storage of their clinic records.",
            "Capture simple maternal health indicators such as age, blood pressure, blood sugar, body temperature, heart rate, gestational stage, symptoms, and visit history.",
            "Implement a machine learning module that classifies patients into low, medium, or high risk levels.",
            "Display flagged high-risk cases clearly for closer staff attention.",
            "Provide patient history and simple report views for monitoring.",
            "Present the system as a decision support tool that assists health workers without replacing medical judgment.",
        ],
    )

    add_heading(document, "Conceptual Review", level=1)
    conceptual_items = [
        (
    ...
    for heading, text in conceptual_items:
        add_heading(document, heading, level=2)
        add_paragraph(document, text)

    add_heading(document, "Literature Review", level=1)
    add_table(
        document,
        ...
        [
            [
                "Salomon et al. (2019)",
                "To assess antenatal care quality and the ability of facilities to detect and manage hypertensive disorders in pregnancy",
                "Cross-sectional facility assessment in Nigerian PHCs and hospitals",
                "Gives direct Nigerian evidence that early maternal risk detection in routine care matters",
                "Does not propose a digital decision support system for PHC staff",
            ],
            [
                "Oyinlola et al. (2025)",
                "To examine regional differences and factors linked to maternal healthcare use in Nigeria",
                "National analytical study using recent survey data",
                "Shows that maternal care use remains low and uneven across Nigeria",
                "Focuses on utilisation patterns, not on staff-facing screening tools",
            ],
            [
                "Oluwatola et al. (2025)",
                "To assess maternal and newborn care quality in PHC facilities across Nigeria",
                "National PHC assessment study",
                "Strongly supports the need for better PHC support tools and monitoring systems",
                "Does not build or test an AI-enabled dashboard for pregnancy risk flagging",
            ],
            [
                "Togunwa et al. (2023)",
                "To classify maternal health risk using machine learning and deep learning methods",
                "Hybrid ANN and random forest model on public maternal risk data",
                "Shows that simple maternal health variables can predict risk categories well",
                "Focuses mainly on model performance, not on a practical PHC software system",
            ],
            [
                "Khadidos et al. (2024)",
                "To predict maternal health risk using ensemble machine learning methods",
                "Ensemble machine learning on maternal health risk data",
                "Shows that maternal risk can be predicted from routine-style features and supports early intervention",
                "Does not package the model into a usable PHC dashboard",
            ],
            [
                "Malde et al. (2025)",
                "To test whether sparse vital-sign data can support maternal risk prediction in lower-middle-income settings",
                "Comparative ML study using simple vital-sign data",
                "Supports the idea that few, easy-to-collect features can still be useful in low-resource settings",
                "Does not focus on local PHC workflow in Nigeria or system implementation",
            ],
        ],
    )
    add_paragraph(
    ...
    add_heading(document, "Gaps Identified vis-a-vis Technology/Method to Resolve the Identified Gaps", level=1)
    add_paragraph(document, "From the reviewed literature, the following gaps can be identified:")
    add_numbered_items(
        document,
        [
            "Many Nigerian maternal health studies describe utilisation or quality problems without translating them into a practical digital support tool for PHC staff.",
            "Many maternal machine learning studies focus on prediction performance instead of system usability in routine care.",
            "Many available solutions are not designed around simple PHC workflow such as patient registration, clinic record entry, flagged-case review, and report generation.",
            "Public datasets exist for maternal risk classification, but there is still a gap in adapting such simple-record models into a local decision support dashboard for Ugbomoro.",
        ],
    )
    add_paragraph(document, "To address these gaps, this project proposes a maternal health decision support system that combines:")
    add_numbered_items(
        document,
        [
            "Staff login and patient registration",
            "Simple clinic record entry using routine maternal indicators",
            "Machine learning based risk classification into low, medium, or high",
            "A flagged-patient dashboard for cases that need attention",
            "Patient history and record review",
            "Simple monitoring reports",
        ],
    )
    add_paragraph(
        document,
    ...
    add_heading(document, "Proposed System Overview", level=1)
    add_paragraph(document, "The proposed system will be a desktop-first web dashboard used mainly by PHC staff. The main modules of the system will include:")
    add_numbered_items(
        document,
        [
            "Staff Login Module: allows authorized staff to sign in to the system.",
            "Patient Registration Module: stores details of pregnant women and creates a patient record.",
            "Clinic Record Entry Module: captures age, blood pressure, blood sugar, body temperature, heart rate, gestational stage, symptoms, and visit history.",
            "Risk Screening Module: uses a simple machine learning model to classify each case as low, medium, or high risk.",
            "Flagged Cases Dashboard: highlights patients with high-risk results so staff can pay closer attention.",
            "Patient History Module: allows staff to review previous entries for a patient.",
            "Basic Reports Module: gives simple counts and summaries of screened patients and flagged cases.",
        ],
    )
    add_paragraph(document, "The system output will be presented as a risk flag or risk screening result, not as a final medical diagnosis.")

    add_heading(document, "Methodology", level=1)
    for paragraph in [
    ...
    add_heading(document, "Expected Result / Outcome", level=1)
    add_paragraph(document, "The proposed study is expected to deliver the following outcomes:")
    add_numbered_items(
        document,
        [
            "Maternal Health Dashboard: a desktop-first web system for PHC staff to register patients and manage maternal clinic records.",
            "Risk Screening Module: a simple machine learning component that classifies records into low, medium, or high risk.",
            "Flagged Case Visibility: a dashboard view that makes possible high-risk pregnancies easier to notice.",
            "Patient Record Support: a structured record history for each registered pregnant woman.",
            "Simple Reporting: basic summaries that help staff monitor the number of screened patients and flagged cases.",
            "Improved Screening Support: a practical decision support tool that can help PHC staff pay earlier attention to possible risk cases in Ugbomoro.",
        ],
    )

    add_heading(document, "Work-Plan / Time Frame Using Gantt Chart", level=1)
    add_paragraph(document, "This research will span for a period of 3 months from March 2026 to May 2026.")
    add_paragraph(document, "The implementation of this project will follow these major stages:")
    add_numbered_items(
        document,
        [
            "Requirement Gathering and Workflow Study: review PHC maternal screening workflow and define system requirements.",
            "Dataset Study and Model Preparation: prepare maternal risk data and train basic classification models.",
            "System Analysis and Design: design the database, dashboard pages, and module flow.",
            "Implementation: build the dashboard, record forms, risk screening logic, and reports.",
            "System Testing and Documentation: test the prototype and prepare screenshots, findings, and final report.",
        ],
    )
    add_table(
        document,
        ["Description", "March 2026", "April 2026", "May 2026"],
        [
            ["Requirement Gathering and Workflow Study", "X", "", ""],
            ["Dataset Study and Model Preparation", "X", "", ""],
            ["System Analysis and Design", "", "X", ""],
            ["Implementation", "", "X", ""],
            ["System Testing and Documentation", "", "", "X"],
        ],
    )

    add_heading(document, "Budget", level=1)
    add_table(
        document,
